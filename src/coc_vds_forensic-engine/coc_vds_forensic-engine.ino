 ...
 * Version: 4.0 "Elite" - Hardened Forensic Build
 * Author: Chinyere Ihuoma Uwa (Kyma)
 * Date: December 2025
 * 
 * * Description:
 * A forensic-grade security monitor for the Arduino Uno R4 WiFi.
 * Implements HMAC-SHA256 hash-chaining, silicon-unique key derivation, 
 * and a fail-closed stealth state machine to enforce custody integrity.
 * Cryptographic audit logs are stored in EEPROM and can be exported
 * via a secure WiFi access point hosting an audit portal.
 * 
 *
 *
 * 
...
enum Custodian : uint8_t { CUST_NONE=0, CUST_HANDLER=1, CUST_RECEIVER=2 };
...
static void reconstructFromLog() {
  ...
  switch (lastEvent) {
    case EVT_CUSTODY_TAKEN: state = IN_CUSTODY; currentCustodian = CUST_HANDLER; break;
    case EVT_TRANSFER_WINDOW_OPEN: state = TRANSFER_WINDOW; currentCustodian = CUST_HANDLER; break;
    case EVT_SEALED:
    case EVT_ADMIN_RESEAL: state = SEALED_IDLE; currentCustodian = CUST_NONE; break;
    case EVT_RECEIVED_FINAL: state = RECEIVED_FINAL; currentCustodian = CUST_RECEIVER; break;
    default: break;
  }
}
...
static int readLight() { return analogRead(LDR_PIN); }
...
static bool isAdminHex(const char* uidHex)   { return (strcmp(uidHex, UID_ADMIN) == 0); }
static bool isHandlerHex(const char* uidHex) { return (strcmp(uidHex, UID_HANDLER) == 0); }
...
static WiFiServer server2(80); // (not used, but keeping to avoid accidental rename collisions)
...
void loop() {
  // Serial fallback: toggle audit
  if (DEBUG_MODE && Serial.available()) {
    char ch = (char)Serial.read();
    if (ch == '\r') { /* ignore */ }
    else if (ch == '\n') {
      serialLine.trim();
      if (serialLine.equalsIgnoreCase("A")) {
        setAuditMode(!auditMode);
      }
      serialLine = "";
    } else {
      serialLine += ch;
      if (serialLine.length() > 64) serialLine.remove(0, serialLine.length() - 64);
    }
  }
  ...
  uint8_t uidBytes[10]; uint8_t uidLen = 0;
  char uidHex[21]; uidHex[0] = '\0';
